...
class Uncertainty(EvaluationVsHighResBase):
    ...
    def compute_residual_histogram(self, dataset: xr.Dataset):
        if dataset.data_vars["residual"].count() == 0:
            out_dataset = xr.DataArray(
                name="n_occurrences", data=np.nan, coords={"residual_bins": [0]}, dims=("residual_bins",)
            )
        else:
    ...
    def count_residual_bin(self, dataset: xr.Dataset):
        return dataset.data_vars["residual"].count().rename("n_occurrences")
